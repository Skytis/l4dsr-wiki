...
This is purely based on Skytis' experience with these Proton versions on Kubuntu 24.04 LTS (x11) between November 2024 and 2025-01-13 and using the bat method. You mileage may vary!

* Proton Experimental: LiveSplit is not splitting accurately, sometimes not at all. Also this Proton branch could get updates (possibly without you noticing) causing more issues
* 9.0-4: LiveSplit is not splitting accurately, sometimes not at all
* 9.0-3: Worked without problems
* 8.0-5: Inputs sometimes don't register, maybe also has the splitting issues
* 7.0-6: Performance leak. Game got increasingly more stutters, even freezing the entire computer once
* 6.3-8: LiveSplit doesn't split (timer does starts correctly though)
* 5.13-6: LiveSplit doesn't start the timer
* 5.0-10: LiveSplit crashes
* 4.11-13: LiveSplit crashes, game is just a black screen (with music)
* 4.2-9: LiveSplit crashes, game is just a black screen (with music)
* 3.16-9: LiveSplit doesn't start (no error window either)
* 3.7-8: LiveSplit doesn't start (no error window either)
* Steam Linux Runtime 1.0 (scout): Nothing starts
* Legacy runtime 1.0: Nothing starts
* GE-Proton9-22: LiveSplit is not splitting accurately, sometimes not at all
* GE-Proton9-21: LiveSplit doesn't start the timer
